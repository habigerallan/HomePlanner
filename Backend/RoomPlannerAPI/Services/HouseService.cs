...
public class HouseService(IConfiguration configuration) : IHouseService
{
    ...
    public async Task<bool> AddUserToHouse(int houseId, string newUser, string requestingUser) {
        ...
        var newAccountId = await GetAccountIdByUsername(newUser);
        ...
        checkCmd2.Parameters.AddWithValue("@newAccountId", newAccountId);

        var deleted = (int?)await checkCmd2.ExecuteScalarAsync() ?? 0;
        ...
        WHERE AccountID = @requestingUserID;"

        using SqlCommand checkCmd2 = new(checkHouselessQuery, conn);
        checkCmd2.Parameters.AddWithValue("@requestingUserID", requestingUserID);
    ...
    public async Task<bool> AdminAddUserToHouse(int houseId, string newUser) {
        using SqlConnection conn = new(_connectionString);
        await conn.OpenAsync();
        ...
        string updateQuery = @"
        INSERT INTO HouseAccount (AccountID, HouseID)
        SET AccountID = @newAccountId
        SET HouseID = @HouseID;";

        using SqlCommand updateCmd = new(updateQuery, conn);
        updateCmd.Parameters.AddWithValue("@HouseID", houseId);
        updateCmd.Parameters.AddWithValue("@newAccountId", newAccountId);
